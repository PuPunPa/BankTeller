...
class SimSystem:
    ...
    def run(self, new_cust, tellers) -> (list, MonitoredResource, float):
        ...
        counter = MonitoredResource('counter', simpy.Resource(self.env, tellers))
        self.env.process(self.source(new_cust, INTERVAL_CUSTOMERS, counter))
        self.env.run()
